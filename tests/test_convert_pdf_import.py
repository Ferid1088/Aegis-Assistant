def test_convert_pdf_importable_as_rag_package_module():
    ...
    importable via the rag package (rag.convert_pdf), not just as a bare top-level
...
def test_ingestion_graph_module_imports_convert_pdf_from_rag_package():
    """Confirms rag/graphs/ingestion.py's import was actually updated, not just that a
    parallel copy exists at rag.convert_pdf while the real call site still uses the
    old, broken bare import."""
    import inspect

    import rag.graphs.ingestion as ingestion_module

    source = inspect.getsource(ingestion_module)
    assert "from rag.convert_pdf import" in source
