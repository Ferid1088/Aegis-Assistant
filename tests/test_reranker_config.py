...
import rag.pipelines.retrieval.nodes as q_nodes
import rag.pipelines.retrieval.state as q
...
def test_get_reranker_passes_fp16_flag():
    ...
    with patch("rag.pipelines.retrieval.state.CrossEncoder") as MockCE, \
         patch("rag.pipelines.retrieval.state.settings") as mock_s, \
         patch("rag.pipelines.retrieval.state.get_device", return_value="cpu"):
...
def test_rerank_impl_uses_batch_size():
    ...
    with patch("rag.pipelines.retrieval.nodes._get_reranker", return_value=mock_reranker), \
         patch("rag.pipelines.retrieval.nodes.settings") as mock_s:
        mock_s.reranker_batch_size = 16
        mock_s.rerank_top_k = 10
        q_nodes._rerank_impl("question", [chunk])
